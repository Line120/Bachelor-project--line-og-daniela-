...
#bb. 4k (egentlig unoedvendig da vi allerede har defineret x som binaer):
#for i in I:
#   for j in J:
#        for k in K:
...
m.setParam('OutputFlag', 1)  # Slaar output til, hvis det er slaaet fra
...
for i in I:
   for j in J:
       for k in K:
           if x[i, j, k].X == 1:
               print(f"Vaerdi af x_{i},{j}^{k}: {x[i, j, k].X}")
