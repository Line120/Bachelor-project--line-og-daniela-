...
fil_sti = "C:/Users/linef/Documents/Bachelor projektet/Datasæt/J_V3_C4.txt"
...
# Opdater alle biler til at have den største kapacitet
...
def generer_ruter(biler, personer, slutdestination, Q):
   ...
   for k, bil in biler.items():
       ...
       for rute_personer in alle_mulige_ruter:
           ...
           omsætning = sum(2 * euklidisk_afstand(personer[c], slutdestination) for c in rute_personer)
           ...
           if samlede_tid <= min_ank_tid and len(rute_personer) <= fri_kapacitet:
               ruter[(k, rute_personer)] = (samlede_tid, omsætning)
...
# Opret variable
x = m.addVars(ruter.keys(), vtype=GRB.BINARY, name="x")


#Definer objektfunktionen
objektfkt =gp.quicksum((omkostning - omsætning) * x[w] for w, (omkostning, omsætning) in ruter.items())
...
# bilen skal vælge en rute hvis der er personer med fra start, ellers kan den vælge en rute
for k, bil in biler.items():
   if bil["occupancy"] > 0:
       m.addConstr(gp.quicksum(x[w] for w in ruter if w[0] == k and isinstance(w[1], tuple)) == 1)
   else:
       m.addConstr(gp.quicksum(x[w] for w in ruter if w[0] == k) <= 1)


# hver person kan samles op maks én gang
for p in personer:
...
for w in ruter:
   if x[w].x > 0.5:
       ...
       print(f"x_w^k = 1, for k = {bilnr}, w = {rute_med_personer}")
